...
class DB(object):
    ...
    数据库工厂
        ...
        oss:
            oss://AccessKeyId:AccessKeySecret@endpoint/db?service_line=test&module=test
        oracle:
            oracle://username:password@ip:port/db?charset=utf8&mode=cx_Oracle.DEFAULT_AUTH
            oracle://sys:chang_on_install@127.0.0.1:1521/orcl?charset=utf8&mode=cx_Oracle.DEFAULT_AUTH
    """

    def __init__(self, **kwargs):
        pass

    def create(self, conn_str: str, *, is_pool=True, **kwargs):
        """
        创建一个数据库连接
        :param conn_str:
        :param kwargs:
        ...
        elif setting_dict["type"].startswith("oss"):
            try:
                from . import db_oss
            except ImportError:
                import db_oss
            self.db_opt = db_oss.OSSOpt(setting_dict, **kwargs)
