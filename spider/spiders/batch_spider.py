# coding:utf8
from spider.spiders import Request, Response, Spider  # isort:skip
import copy
import datetime
import json
import time
...
from spider import setting, util
...
class JsonTask(object):
    ...
    JsonTask对象
        tips:
            不支持通过属性赋值添加新任务字段
            但  可以通过update方法 类似 字典的update来进行赋值新字段

    将常规任务 {"name": "a", "age": 1} 进行封装 提供常规便利操作
    To use:
    ...
    def copy(self, *, keep_retry=False, update: dict = None):
        """
            返回一个和自己一样的对象
        Args:
            keep_retry: 是否保留retry字段 默认False
            update: 快捷操作 以字典形式指定需要更新的字段key和value  一般需要copy的场景都需要修改一些参数
                例如:
    ...
    def generate_tasks(
        ...
        """
            针对翻页任务 提供接口 自动生成指定页码数的任务
                页码范围: [start, end)
        Args:
            field: 页码字段名 默认为page
            start: 起始页码
            end: 结束页码 不包含
            is_obj: 是否返回 JsonTask 对象
            json_dumps: 是否使用json序列化
            keep_retry: 是否保留当前retry值
...
class SingleBatchSpider(Spider):
    """
        单批次爬虫
            使用MySQL作为任务表 持久化存储任务防止丢失
    """

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        logger.debug("BatchSpider:Config:{}".format(kwargs))
        # 默认数据库
    ...
    def check_lost_task(self):
        ...
        sql = f"""update {self.task_table_name} 
                    set {self.state_field_name}={self.state_dict['task_wait']} 
                    where {self.state_field_name}={self.state_dict['task_run']} 
    ...
    def get_task(
        self, obj: bool = False, group: bool = False, **kwargs
    ) -> Union[str, JsonTask]:
        """
        获取任务执行
        Args:
        ...
        task = self._get_task_from_redis()
        if not task:
    ...
    def get_task_from_mysql(self, redis_lock=None, **kwargs) -> int:
        ...
        while task_mysql_limit > 0:
            ...
            logger.info(
                "查询到未做任务记录 {} 条,耗时 {} s".format(len(sql_result), _query_use_time)
            )
            if not sql_result:
                break
            ...
            if task_list:
                ...
                del task_list
        _db.close()
        logger.info("本次从mysql获取到任务共 {} 条".format(current_get_task_count))
    ...
    def put_task(
        ...
        """
            任务加入队列
        Args:
            task: json字符串 or  JsonTask 对象  或者list中包含上述对象
            retry: 是否为重试任务
            show_log:
            task_key:
            message: 日志
        ...
        example_task = task_list[0] if task_list_len == 1 else task_list
        if show_log:
            message = message or ("任务重试" if retry else "任务新增")
            logger.debug(f"{message}: {task_list_len} {example_task}")
        #
        if self.cache_task:
            while task_list:
                try:
    ...
    def set_task_state(
        ...
        assert isinstance(state, int), "任务状态必须为整型"
    ...
    def send_message(self, message):
        ...
        return util.send_message(message, names=self.message_recipients)
    ...
    def _get_task_from_redis(self, delay=30, task_key: str = None):
        """
        从redis中获取任务
            或者从缓存队列中取
        Args:
    ...
    def _send_spider_start_signal(self):
        ...
        """
        if self.debug or not setting.IS_SPIDER:
...
class BatchSpider(SingleBatchSpider):
    ...
    def _check_batch(self):
        ...
        if self.batch_complete(total_count, done_count):
            logger.debug("上批次 {} 已完成".format(batch_date_str))
            # 清理残留任务
